...
(function () {
  try {
    ...
    fab.addEventListener('click', () => {
      if (chrome.sidePanel) {
        chrome.sidePanel.open({ path: 'sidepanel.html' }).catch((err) => {
          console.error('FAB: sidePanel error:', err.message);
        });
      } else {
        // Fallback: try sending message to background
        chrome.runtime.sendMessage({ type: 'open.sidepanel' }, () => {
          if (chrome.runtime.lastError) {
            // No background — notify user
            alert('OpenAgent: Background script not loaded. Try reloading the extension in chrome://extensions');
          }
        });
      }
    });
    ...
    console.error('FAB injection failed:', e);
  }
})();
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  switch (message.type) {
    case 'page.collect':
      collectPageProbe()
        .then(sendResponse)
        .catch((err) => sendResponse({ error: err.message, rawCapture: { metadata: pageMetadata() } }));
      return true;
    ...
    case 'page.dom.perform':
      performDomActions(message.steps)
        .then(sendResponse)
        .catch((err) => sendResponse({ ok: false, summary: err.message, results: [] }));
      return true;
...
async function collectPageProbe() {
  ...
  return {
    rawCapture: {
      ...
      images,
      privacyFlags: {
        containsSensitiveFormData: !!document.querySelector(
          'input[type="password"], input[autocomplete*="cc-"], input[autocomplete*="card"]'
        ),
    ...
    features: {
      ...
      hasDenseInteractiveUi:
        document.querySelectorAll('button, input, [role="button"]').length > 15,
    },
  };
}
...
const HIGH_VALUE_SELECTORS = [
  ...
  '.post', '.article', '.content', '.entry', '.story',
];

const VISIBLE_TEXT_SELECTORS = [
  'section', 'div', 'p', 'li', 'td', 'th',
  'h1', 'h2', 'h3', 'h4', 'h5', 'h6',
];
...
function collectBodyText() {
  ...
  for (const sel of VISIBLE_TEXT_SELECTORS) {
    for (const el of document.querySelectorAll(sel)) {
      if (isVisible(el) && el.textContent.trim().length > 50) {
        candidates.push({ el, priority: 1 });
      }
    }
  }
  ...
  // Sort: high-priority first, then by position
...
function collectDomSnapshot() {
  ...
  const selectors = [
    ...
    'summary', '[role="button"]', '[role="link"]', '[role="menuitem"]',
  ...
  const described = elements
    .filter((el) => !domElementRefs.has(el)) // dedupe
    .slice(0, 60)
    .map((el, i) => {
      const ref = `el-${i + 1}`;
  ...
  return {
    metadata: pageMetadata(),
    elements: described,
    capabilities: { supportsDomAutomation: true },
  };
}
...
function describeElement(ref, el) {
  ...
  return {
    ...
    value: isInput && el.value ? el.value : undefined,
    placeholder: isInput && el.placeholder ? el.placeholder : undefined,
    type: isInput && el.type ? el.type : undefined,
    isTextEntryCandidate: isTextEntryCandidate(el),
...
function isTextEntryCandidate(el) {
  return (
    ...
    !['hidden', 'checkbox', 'radio', 'file', 'submit', 'button', 'image', 'reset'].includes(el.type)
  ) || el instanceof HTMLTextAreaElement || el.getAttribute('contenteditable') === 'true';
}
...
async function performDomActions(steps) {
  ...
  for (const step of steps.slice(0, MAX_STEPS)) {
    const result = await performDomAction(step);
    results.push(result);
  }
...
function resolveTarget(step) {
  ...
  if (step.label) {
    ...
    for (const [ref, el] of domElementRefs) {
      if (getElementLabel(el).toLowerCase().includes(normalized) ||
          (el.innerText || '').toLowerCase().includes(normalized)) {
        return el;
      }
    }
  }
...
function handleNavigation(command) {
  if (command.kind === 'scroll') {
    ...
    if (!/^https?:\/\//.test(url)) return { ok: false, error: 'Only HTTP(S) URLs supported' };
